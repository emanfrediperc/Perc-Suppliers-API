...
export class ReporteController {
  ...
  async exportPagosPorProveedor(
    ...
    const totalsRow = {
      razonSocial: 'TOTAL',
      cantidadPagos: data.proveedores.reduce(
        (s: number, p: any) => s + (p.cantidadPagos || 0),
        0,
      ),
      montoBase: data.proveedores.reduce(
        (s: number, p: any) => s + (p.montoBase || 0),
        0,
      ),
      montoNeto: data.proveedores.reduce(
        (s: number, p: any) => s + (p.montoNeto || 0),
        0,
      ),
    };
    await this.sendExport(
  ...
  @Roles('admin', 'tesoreria', 'operador', 'consulta')
  async exportRetencionesAcumuladas(
    @Query() dto: ReporteQueryDto,
    @Res() res: express.Response,
  ) {
  ...
  async exportFacturasPorTipo(
    ...
    const totalsRow = {
      tipo: 'TOTAL',
      cantidad: data.tipos.reduce(
        (s: number, t: any) => s + (t.cantidad || 0),
        0,
      ),
      montoTotal: data.tipos.reduce(
        (s: number, t: any) => s + (t.montoTotal || 0),
        0,
      ),
      montoNeto: data.tipos.reduce(
        (s: number, t: any) => s + (t.montoNeto || 0),
        0,
      ),
      montoIva: data.tipos.reduce(
        (s: number, t: any) => s + (t.montoIva || 0),
        0,
      ),
    };
    await this.sendExport(
  ...
  async exportEstadoCuenta(
    ...
    if (!dto.empresaProveedora) {
      return res
        .status(400)
        .json({ message: 'Falta el parámetro empresaProveedora' });
    }
    const data = await this.service.getEstadoCuentaCompleto(
      dto.empresaProveedora,
    );
    if (!data.proveedor) {
    ...
    const resumenRows = [
      ...
      { campo: '', valor: '' },
      {
        ...
        valor: `$ ${fmtMoney(data.totales.facturado)}`,
      },
      { campo: 'Total Pagado', valor: `$ ${fmtMoney(data.totales.pagado)}` },
      {
        campo: 'Saldo Pendiente',
        valor: `$ ${fmtMoney(data.totales.saldoPendiente)}`,
      },
      {
        campo: 'Total NC / ND',
        valor: `$ ${fmtMoney((data.totales as any).totalNC || 0)}`,
      },
    ];
    ...
    const facturasCols: ExportColumn[] = [
      ...
      {
        header: 'Cliente',
        key: 'empresaCliente.razonSocial',
        type: 'text',
        width: 28,
      },
    ...
    const facturasSheet: WorkbookSheetConfig = {
      ...
      totalsRow: {
        numero: 'TOTAL',
        montoTotal: data.facturas.reduce(
          (s: number, f: any) => s + (f.montoTotal || 0),
          0,
        ),
        montoPagado: data.facturas.reduce(
          (s: number, f: any) => s + (f.montoPagado || 0),
          0,
        ),
        saldoPendiente: data.facturas.reduce(
          (s: number, f: any) => s + (f.saldoPendiente || 0),
          0,
        ),
      },
    };
    ...
    const pagosSheet: WorkbookSheetConfig = {
      ...
      totalsRow: {
        fechaPago: 'TOTAL',
        montoBase: data.pagos.reduce(
          (s: number, p: any) => s + (p.montoBase || 0),
          0,
        ),
        montoNeto: data.pagos.reduce(
          (s: number, p: any) => s + (p.montoNeto || 0),
          0,
        ),
      },
    ...
    const ncSheet: WorkbookSheetConfig = {
      ...
      totalsRow: {
        numero: 'TOTAL',
        montoTotal: data.notasCredito.reduce(
          (s: number, n: any) => s + (n.montoTotal || 0),
          0,
        ),
      },
    };
    ...
    if (dto.formato === 'csv') {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename=estado-cuenta-${prov.razonSocial.replace(/\s+/g, '_')}.csv`,
      );
      return res.send(csv);
    }
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=estado-cuenta-${prov.razonSocial.replace(/\s+/g, '_')}.xlsx`,
    );
  ...
  @Roles('admin', 'tesoreria', 'operador', 'consulta')
  async exportComisionesDescuentos(
    @Query() dto: ReporteQueryDto,
    @Res() res: express.Response,
  ) {
    ...
    const totalsRow = {
      razonSocial: 'TOTAL',
      montoBase: data.porProveedor.reduce(
        (s: number, p: any) => s + (p.montoBase || 0),
        0,
      ),
      comision: data.porProveedor.reduce(
        (s: number, p: any) => s + (p.comision || 0),
        0,
      ),
      descuento: data.porProveedor.reduce(
        (s: number, p: any) => s + (p.descuento || 0),
        0,
      ),
    };
    await this.sendExport(
  ...
  @Roles('admin', 'tesoreria', 'operador', 'consulta')
  async exportFacturasVencimiento(
    @Query() dto: ReporteQueryDto,
    @Res() res: express.Response,
  ) {
    ...
    const totalsRow = {
      ...
      montoTotal: rows.reduce(
        (s: number, r: any) => s + (r.montoTotal || 0),
        0,
      ),
      saldoPendiente: rows.reduce(
        (s: number, r: any) => s + (r.saldoPendiente || 0),
        0,
      ),
    };
    await this.sendExport(
  ...
  @Roles('admin', 'tesoreria', 'operador', 'consulta')
  getPagosPorPeriodo(@Query() dto: ReporteQueryDto) {
    return this.service.getPagosPorPeriodo(dto);
  }

  @Get('pagos-por-proveedor')
  @Roles('admin', 'tesoreria', 'operador', 'consulta')
  getPagosPorProveedor(@Query() dto: ReporteQueryDto) {
    return this.service.getPagosPorProveedor(dto);
  }

  @Get('facturas-vencimiento')
  @Roles('admin', 'tesoreria', 'operador', 'consulta')
  getFacturasVencimiento(@Query() dto: ReporteQueryDto) {
    return this.service.getFacturasVencimiento(dto);
  }

  @Get('retenciones-acumuladas')
  @Roles('admin', 'tesoreria', 'operador', 'consulta')
  getRetencionesAcumuladas(@Query() dto: ReporteQueryDto) {
    return this.service.getRetencionesAcumuladas(dto);
  }

  @Get('comisiones-descuentos')
  @Roles('admin', 'tesoreria', 'operador', 'consulta')
  getComisionesDescuentos(@Query() dto: ReporteQueryDto) {
    return this.service.getComisionesDescuentos(dto);
  }

  @Get('estado-cuenta-proveedor')
  @Roles('admin', 'tesoreria', 'operador', 'consulta')
  async getEstadoCuentaProveedor(
    @Query() dto: ReporteQueryDto,
  ...
  getEstadoCuentaCompleto(
    @Param('empresaProveedoraId') empresaProveedoraId: string,
  ) {
  ...
  @Roles('admin', 'tesoreria', 'operador', 'consulta')
  getFacturasPorTipo(@Query() dto: ReporteQueryDto) {
    return this.service.getFacturasPorTipo(dto);
  }
  ...
  private async sendExport(
    ...
    try {
      ...
      if (!res.headersSent) {
        res
          .status(500)
          .json({ message: 'Error generating export', error: error.message });
      }
    }
  }
}
