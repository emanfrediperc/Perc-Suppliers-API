...
export class PagoProgramadoController {
  ...
  findAll(
    ...
    // String() evita NoSQL injection en el filtro estado (?estado[$ne]=...).
  ...
  async export(
    ...
    const columns: ExportColumn[] = [
      { header: 'Fecha Programada', key: 'fechaProgramada', type: 'datetime' },
      {
        ...
        key: 'ordenPago.numero',
        type: 'text',
        width: 22,
      },
      {
        header: 'Proveedor',
        key: 'ordenPago.empresaProveedora.razonSocial',
        type: 'text',
        width: 32,
      },
    ...
    if (formato === 'csv') {
      ...
      res.setHeader(
        'Content-Disposition',
        'attachment; filename=pagos-programados.csv',
      );
      res.send(csv);
    } else {
      ...
      res.setHeader(
        'Content-Disposition',
        'attachment; filename=pagos-programados.xlsx',
      );
      res.send(buffer);
    }
  }
