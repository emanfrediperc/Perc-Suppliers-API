...
} from './schemas/empresa-proveedora.schema';
...
import { cifrarDatosBancarios } from '../../common/utils/datos-bancarios.util';
...
export class EmpresaProveedoraService {
  constructor(
    ...
    private empresaModel: Model<EmpresaProveedoraDocument>,
    @Inject('FINNEGANS_SERVICE') private finnegansService: IFinnegansService,
  ) {}

  async create(
    dto: CreateEmpresaProveedoraDto,
  ): Promise<EmpresaProveedoraDocument> {
    ...
    const finnegansResult = await this.finnegansService.createCompanyInERP({
      cuit: dto.cuit,
      razonSocial: dto.razonSocial,
    });
    return this.empresaModel.create({
      ...dto,
      datosBancarios: cifrarDatosBancarios(dto.datosBancarios),
      finnegansId: finnegansResult.finnegansId,
    });
  }
  ...
  async findAll(
    ...
    sinConvenio = false,
  ): Promise<PaginatedResponseDto<EmpresaProveedoraDocument>> {
    ...
    if (search) {
      ...
      filter.$or = [
        ...
        { nombreFantasia: { $regex: escaped, $options: 'i' } },
      ];
    }
    if (sinConvenio) {
      filter.convenios = { $size: 0 };
    }
  ...
  async findOne(id: string): Promise<EmpresaProveedoraDocument> {
    ...
    if (!empresa)
      throw new NotFoundException('Empresa proveedora no encontrada');
    return empresa;
  }
  ...
  async update(
    id: string,
    dto: UpdateEmpresaProveedoraDto,
  ): Promise<EmpresaProveedoraDocument> {
    // Cifrar el CBU at-rest si viene en el patch.
    const patch = dto.datosBancarios
      ? { ...dto, datosBancarios: cifrarDatosBancarios(dto.datosBancarios) }
      : dto;
    const empresa = await this.empresaModel.findByIdAndUpdate(id, patch, {
      new: true,
    });
    if (!empresa)
      throw new NotFoundException('Empresa proveedora no encontrada');
  ...
  async setApocrifoOverride(
    ...
    if (activo) {
      if (!motivo)
        throw new NotFoundException('Motivo requerido para activar override');
    ...
    const empresa = await this.empresaModel.findByIdAndUpdate(id, update, {
      new: true,
    });
    if (!empresa)
      throw new NotFoundException('Empresa proveedora no encontrada');
