...
import { descifrarCbu } from '../../common/utils/datos-bancarios.util';
...
export class PagoService {
  ...
  async findAll(
  ...
  ): Promise<PaginatedResponseDto<PagoDocument>> {
    ...
    const [data, total] = await Promise.all([
      this.pagoModel
        .find()
        .populate({ path: 'factura', populate: { path: 'empresaProveedora' } })
  ...
  async findOne(id: string): Promise<PagoDocument> {
    const pago = await this.pagoModel
      ...
      .populate({
        path: 'factura',
        populate: [{ path: 'empresaProveedora' }, { path: 'empresaCliente' }],
      })
      .populate('convenioAplicado');
  ...
  async anular(id: string): Promise<PagoDocument> {
    ...
    try {
      ...
      if (factura) {
        const pagosActivos = await this.pagoModel.find({
          factura: factura._id,
          estado: { $ne: 'anulado' },
        });
        const montoPagado = pagosActivos.reduce(
          (sum, p) => sum + p.montoBase,
          0,
        );
        ...
        if (factura.ordenPago) {
          const orden = await this.ordenModel
            .findById(factura.ordenPago)
            .populate('facturas');
          if (orden) {
            const pagosActivosOrden = await this.pagoModel.find({
              ordenPago: orden._id,
              estado: { $ne: 'anulado' },
            });
            orden.montoPagado = pagosActivosOrden.reduce(
              (sum, p) => sum + p.montoBase,
              0,
            );
            orden.saldoPendiente = Math.max(
              0,
              orden.montoTotal - orden.montoPagado,
            );

            const allPaid = (orden.facturas as any[]).every(
              (f: any) => f.estado === 'pagada',
            );
            const anyPaid = (orden.facturas as any[]).some(
              (f: any) => f.estado === 'pagada' || f.estado === 'parcial',
            );
  ...
  async generateComprobante(id: string): Promise<Buffer> {
    const pago = await this.pagoModel
      ...
      .populate({
        path: 'ordenPago',
        populate: [
          { path: 'empresaProveedora' },
          { path: 'facturas', populate: { path: 'empresaCliente' } },
        ],
      })
      .populate('convenioAplicado');
    if (!pago) throw new NotFoundException('Pago no encontrado');
    ...
    return new Promise((resolve) => {
      ...
      const cliente =
        factura?.empresaCliente || orden?.facturas?.[0]?.empresaCliente;
      ...
      doc
        ...
        .font('Helvetica')
        .fillColor('#666')
        .text(`Fecha de emision: ${new Date().toLocaleDateString('es-AR')}`, {
          align: 'center',
        });
      doc.moveDown(1.5);
      ...
      if (proveedor) {
        ...
        if (proveedor.condicionIva)
          doc.text(`Condicion IVA: ${proveedor.condicionIva}`);
        ...
        if (proveedor.datosBancarios) {
          ...
          if (db.cbu) doc.text(`CBU: ${descifrarCbu(db.cbu)}`);
      ...
      if (cliente) {
        ...
        if (cliente.condicionIva)
          doc.text(`Condicion IVA: ${cliente.condicionIva}`);
      ...
      doc.fontSize(10).font('Helvetica');
      if (factura)
        doc.text(`Factura: ${factura.numero || '-'}  (${factura.tipo || '-'})`);
      if (orden) doc.text(`Orden de Pago: ${orden.numero || '-'}`);
      doc.text(
        `Fecha de Pago: ${pago.fechaPago ? new Date(pago.fechaPago).toLocaleDateString('es-AR') : '-'}`,
      );
      ...
      doc.moveDown(0.3);
      doc.fontSize(10).font('Helvetica');
      const rows: [string, string][] = [['Monto Base', fmt(pago.montoBase)]];
      const totalRetenciones =
        (pago.retencionIIBB || 0) +
        (pago.retencionGanancias || 0) +
        (pago.retencionIVA || 0) +
        (pago.retencionSUSS || 0) +
        (pago.otrasRetenciones || 0);
      if (pago.retencionIIBB)
        rows.push(['Retencion IIBB', `- ${fmt(pago.retencionIIBB)}`]);
      if (pago.retencionGanancias)
        rows.push(['Retencion Ganancias', `- ${fmt(pago.retencionGanancias)}`]);
      if (pago.retencionIVA)
        rows.push(['Retencion IVA', `- ${fmt(pago.retencionIVA)}`]);
      if (pago.retencionSUSS)
        rows.push(['Retencion SUSS', `- ${fmt(pago.retencionSUSS)}`]);
      if (pago.otrasRetenciones)
        rows.push(['Otras Retenciones', `- ${fmt(pago.otrasRetenciones)}`]);
      if (totalRetenciones > 0)
        rows.push(['Total Retenciones', `- ${fmt(totalRetenciones)}`]);
      if (pago.comision)
        rows.push([
          `Comision (${pago.porcentajeComision}%)`,
          `- ${fmt(pago.comision)}`,
        ]);
      if (pago.descuento)
        rows.push([
          `Descuento (${pago.porcentajeDescuento}%)`,
          `- ${fmt(pago.descuento)}`,
        ]);
      ...
      doc.text('MONTO NETO TRANSFERIDO', 70, doc.y, {
        continued: true,
        width: 300,
      });
